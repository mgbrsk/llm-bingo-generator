...
class BingoPipeline:
    ...
    def create_bingo_card(self, cell_card_amount: int, texts: str):
        # Первоначальный промт пользователя.
        ...
        # )
        prompt_string = (
            f"Придумай {variant_amount} уникальных коротких и ироничных идей на тему '{texts}'. "
            "Не используй примеры из описания. "
        ...
        if len(splitted_response[0:variant_amount]) != variant_amount:
            # Пробуем почистить несколько раз, но сложно.
            messages_list = [
                {
                    "role": "system",
                    "content": f"Ты форматируешь текст. Убери все цифры из текста, доступен ответ только на русском языке. Доступный ответ ТОЛЬКО объект type(list). Длинна списка должна быть {variant_amount}.",
                },
                {
                    "role": "user",
